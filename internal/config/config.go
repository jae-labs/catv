// Package config manages application configuration for CATV
package config
...
// Config holds all configuration for the CATV application
type Config struct {
	// Database settings
	DatabasePath string
...
func DefaultConfig() *Config {
	...
	dataDir := filepath.Join(homeDir, ".catv")
...
func LoadConfig() *Config {
	...
	if model := os.Getenv("CATV_MODEL"); model != "" {
		cfg.OllamaModel = model
	}

	if url := os.Getenv("CATV_OLLAMA_URL"); url != "" {
		cfg.OllamaURL = url
	}

	if dataDir := os.Getenv("CATV_DATA_DIR"); dataDir != "" {
		cfg.DataDir = dataDir
